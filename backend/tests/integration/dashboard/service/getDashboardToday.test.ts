...
describe('Dashboard - Today', () => {
  it('should return today stats with appointments', async () => {
    ...
    const now = new Date();
    const start = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 10).toISOString();
    const end = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 11).toISOString();

    await createAppointment(token, pet, service, start, end);

    const res = await apiRequest.get('/dashboards/today').set('Authorization', `Bearer ${token}`);
  ...
  it('should return zero stats when no appointments today', async () => {
    const token = await createUser();
